...
public class FederationRepository {
    ...
    public void saveMemberReferees(String memberId, Map<String, String> refereeRelationshipNature) {
        if (refereeRelationshipNature == null) {
            return;
        }
        for (Map.Entry<String, String> entry : refereeRelationshipNature.entrySet()) {
            jdbc.update(
                "INSERT INTO member_referees (member_id, referee_id, relationship_nature) VALUES (?, ?, ?)",
                memberId,
                entry.getKey(),
                entry.getValue()
            );
    ...
    public Collectivity saveCollectivity(Collectivity c) {
        ...
        if (existing != null && existing > 0) {
            ...
            jdbc.update("DELETE FROM collectivity_members WHERE collectivity_id = ?", c.getId());
        } else {
            jdbc.update(
                "INSERT INTO collectivity (id, name, location, specialty, creation_date, president_id, vice_president_id, treasurer_id, secretary_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                c.getId(), c.getName(), c.getLocation(), c.getSpecialty(), c.getCreationDate(),
                c.getStructure().getPresident().getId(),
                c.getStructure().getVicePresident().getId(),
                c.getStructure().getTreasurer().getId(),
                c.getStructure().getSecretary().getId()
